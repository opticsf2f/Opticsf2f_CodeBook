...
class BlochSphere:
    ...
    def addAxisArrow(self, x,y,z, color=cDUbbbb):
        """
            Adds state arrow to the Bloch sphere, given the tip position.
        """
        length = np.sqrt(x*x + y*y + z*z)
        arrow=pv.Arrow(start=(0.0, 0.0, 0.0), direction=np.array([x,y,z]) * self.r,
                       tip_length=0.05/length, tip_radius=0.02/length, tip_resolution=20,
                       shaft_radius=0.01/length, shaft_resolution=20, scale=length * self.r)
        self.p.add_mesh(arrow, opacity=1.0, color=color, smooth_shading=True)
    ...
    def addLine(self, trajectoryXYZ):
        """
            Adds trajectory in time, with time shown with viridis colouring.
        """
        spline = pv.Spline(trajectoryXYZ * self.r, 1000)
        spline["scalars"] = np.arange(spline.n_points)
        tubes=spline.tube(radius=0.02)
        self.p.add_mesh(tubes, smooth_shading=True, color = cDUk, show_scalar_bar=False)
